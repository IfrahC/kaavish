...
\subsubsection{Admin}
Admins are system controllers responsible for operational oversight. Admins manage:
\begin{itemize}
    \item User accounts
...
\begin{itemize}
    \item Stores: attendance, project submissions, lesson rating, assessment scores, instructor comments
    \item Functions:
    \begin{itemize}
...
\subsubsection{NLP + LLM Evaluation Model}
NLP + LLM -based subsystem analyzing class recordings:
\begin{itemize}
    \item \texttt{analyzeRecording()}
    \item \texttt{extractTranscript()}
    \item \texttt{scoreMetrics()}
    \item \texttt{generateReport()}
\end{itemize}
...
This section presents the computational, algorithmic, and architectural components that form the core of TeachWise. Unlike conventional CRUD-based systems, TeachWise integrates intelligent scheduling, AI-based instructor evaluation, automated Zoom meeting generation, and multimodal data pipelines. For each subsystem, we describe the inputs, internal processes, and outputs, along with how these components integrate into the system-wide toolchain.
\\
\\
...
The platform uses an intelligent scheduling engine inspired by hybrid optimization techniques from recent literature \cite{Lv2025, Sahargahi2021, Nguyen2021}.
...
\begin{enumerate}
    ...
    Generates multiple candidate schedules and evolves them using selection, mutation, and crossover \cite{Sahargahi2021}.

    \item \textbf{Local Search Optimization:}  
    Refines feasible schedules to minimize soft constraint violations \cite{Sahargahi2021}.

    \item \textbf{Distance-to-Feasibility Fitness Function:}  
    Ensures hard constraints (Zoom limits, overlapping classes, instructor conflicts) are always satisfied while optimizing soft constraints such as preferences, continuity, and compatibility \cite{Sahargahi2021}.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Scheduling Layer (Python):}  
